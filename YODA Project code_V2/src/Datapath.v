...
module Datapath (
    ...
    wire [3:0]  alu_control;            // ALU function select from ALU_Control - Extended: was [2:0], now [3:0] for FPC-16
    ...
    assign reg_write_data = io_read    ? io_in          // I/O input port (LD from 0x00FE)
                      : mem_to_reg ? mem_read_data   // normal LD from data memory
                      :              alu_result;      // R-type / compute result
