...
## FPC-16 Extension
### Overview
FPC-16 extends the baseline StarCore-1 processor with native fixed-point arithmetic and memory-mapped I/O support.

The original StarCore-1 processor only supported integer arithmetic, which limited its suitability for practical embedded applications involving fractional sensor values. FPC-16 addresses this limitation by introducing:

Native Q8.8 fixed-point arithmetic
Fixed-point multiplication `FPMUL`
Fixed-point division `FPDIV`
Optional signed fixed-point operation support
`Memory-mapped I/O` ports for external peripheral interaction

The processor remains a 16-bit single-cycle Harvard architecture while preserving backward compatibility with all existing StarCore-1 instructions.

## Q8.8 Fixed-Point Representation

| Bits     | Purpose              |
| -------- | -------------------- |
| `[15:8]` | Integer component    |
| `[7:0]`  | Fractional component |

A value is interpreted as:
...
## Extended ISA - FPC-16 Instructions
Two previously unused opcode slots were assigned to fixed-point arithmetic operations.

| Opcode | Mnemonic | Type   | Description               |
| ------ | -------- | ------ | ------------------------- |
| `1110` | FPMUL    | R-type | Fixed-point Q8.8 multiply |
| `1111` | FPDIV    | R-type | Fixed-point Q8.8 divide   |


## Fixed-Point Multiplication

Two Q8.8 operands are multiplied:
`Q8.8 × Q8.8 → Q16.16 intermediate product`

The multiplication produces a 32-bit result: `fp_mul_full = a * b`

The correctly scaled Q8.8 result is extracted using:
`result = fp_mul_full[23:8]`

Example:
`1.5 × 2.5

0x0180 × 0x0280 = 0x0003C000

Extract [23:8] → 0x03C0 = 3.75

## Fixed Point Division

To preserve fractional precision during division, the dividend is pre-scaled before division:

`(a << 8) / b`

Implemented internally as:

`fp_div_shifted = {a, 8'b0}`
...
0x030000 / 0x0200 = 0x0180 = 1.5

## Signed Fixed-Point Support

The ALU includes optional signed fixed-point support using the `signed_mode` input.

When:
`signed_mode = 1'b1`

FPMUL and FPDIV interpret operands as signed two’s complement Q8.8 values.

## Memory-Mapped I/O Extension

FPC-16 introduces memory-mapped I/O support to allow interaction with external peripherals using standard `LD` and `ST` instructions.

Address Map:
| Address  | Function               |
| -------- | ---------------------- |
| `0x0008` | Input Port (`io_in`)   |
| `0x0009` | Output Port (`io_out`) |


A `LD` instruction targeting address 0x0008 injects external input data into the datapath.

Example:
`LD R5, [R4+0]`

A `ST` instruction targeting address 0x0009 writes data to the external output register.
...
Modified Write-Back Multiplexer
...
The FPC-16 extension was verified using:

ALU-level testbenches
ALU control decoder tests
Control unit tests
Full processor integration simulation
GTKWave timing analysis

Verification includes:

Q8.8 multiplication correctness
Opcode decoding
Register write-back correctness
Memory-mapped I/O operation
End-to-end datapath execution
